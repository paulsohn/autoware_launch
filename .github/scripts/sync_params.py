...
def _extract_comment_text(comment_obj: Any) -> str:
    ...
    Example:
        comment_obj.value = "# {OVERRIDE: tuned}"
        returns "{OVERRIDE: tuned}"
    """
    comment_value = getattr(comment_obj, "value", str(comment_obj))
...
def _find_flow_sequence_end_idx(lines: list[str], start_idx: int) -> int:
    ...
    Example:
        lines = ["items:", "  [1,", "   2,", "   3]  # tail", "other: 5"]
        start_idx = 1
        returns 4 (index after the closing bracket line)
...
def _build_key_line_index(text: str) -> dict[tuple[str, ...], KeyLineInfo]:
    ...
    Example:
        text = "a: 1\nparent:\n  child: 2\n"
        returns {("a",): KeyLineInfo(line_idx=0, indent=0, has_inline_value=True),
                 ("parent",): KeyLineInfo(line_idx=1, indent=0, has_inline_value=False),
                 ("parent", "child"): KeyLineInfo(line_idx=2, indent=2, has_inline_value=True)}
    """
...
def ensure_override_markers_in_text(
    ...
    Example:
        text = "a: 1\n"
        override_comments = {("a",): "{OVERRIDE}"}
        returns "a: 1 # {OVERRIDE}\n"
    """
    lines = text.splitlines()
    key_index = _build_key_line_index(text)
...
def _parse_repository_entries(entries_data: Any, context: str) -> list[RepositoryEntry]:
    ...
    Example:
        entries_data = [{"repository": "org/repo", "files": []}]
        context = "category 'perception'"
        returns [RepositoryEntry(repository="org/repo", ref=None, files=())]
    """
...
def load_config(config_path: Path, category: str) -> list[RepositoryEntry]:
    ...
    Example:
        config_path points to a file with category "perception"
        category = "perception"
...
def clone_repository(repo: str, ref: str, temp_root: Path) -> Path:
    ...
    Example:
        repo = "org/repo"
        ref = "main"
        returns Path("/tmp/.../org__repo_<hash>")
    """
...
def source_blob_url(source_repo: str, source_sha: str, source_path: str) -> str:
    ...
    Example:
        ...
        source_sha = "abc"
        source_path = "config/file.yaml"
        returns "https://github.com/org/repo/blob/abc/config/file.yaml"
    """
    return f"https://github.com/{source_repo}/blob/{source_sha}/{source_path}"


def build_variant_header(source_url: str, category: str) -> str:
    ...
    Example:
        source_url = "https://github.com/org/repo/blob/abc/config/file.yaml"
    ...
    lines = [
        ...
        f"# 2. Run the sync-params workflow or run `python .github/scripts/sync_params.py {category}` to update this file.",
...
def extract_pinned_source_sha_and_path(variant_text: str) -> tuple[str, str] | None:
    ...
    Example:
        variant_text = "# Source: https://github.com/org/repo/blob/abc/config/file.yaml\n"
        returns ("abc", "config/file.yaml")
    """
...
def extract_override_values(
    ...
    Example:
        variant_path contains "a: 1 # {OVERRIDE}"
        returns ({("a",): 1}, {("a",): "{OVERRIDE}"})
    """
...
def _build_unified_diff(path: Path, old_content: str | None, new_content: str) -> str:
    ...
    *old_content* is ``None`` the diff is shown as a pure addition.
    ...
    Example:
        old_content = "a\n"
        new_content = "b\n"
        returns a unified diff string containing "-a" and "+b"
    """
...
def sync_file_entry(
    ...
    Example:
        check = True
        returns 0 when no variants drift
    """
    ...
    for variant in file_entry.variants:
        ...
        if not overrides:
            variant_content = (
                build_variant_header(effective_source_url, category) + effective_source_body
            )
        ...
        variant_content = build_variant_header(effective_source_url, category) + variant_body
        variant_content += effective_embedded_original
        changed = write_or_check(variant_abs, variant_content, check)
        if changed:
            variants_changed += 1
...
def main() -> int:
    """Parse CLI arguments and run the sync pipeline.
